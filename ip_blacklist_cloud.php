...
function ip_added()
{
	...
	$IP=mysql_real_escape_string($_GET['blacklist']);
...
function IPBLC_IP_value( $column, $comment_ID )
{
	...
	if ( 'IPBLC_IP_status' == $column )
	{
		...
		else
		{
			...
			$authorName2=mysql_real_escape_string($authorName2);
...
$link="http://www.ip-finder.me/wp-content/themes/ipfinder/blacklist_delete.php?IP=".$IP."&website=".urlencode(site_url())."&website_name=".urlencode(get_bloginfo('name'));					$post_to_cloud =  file_get_contents (
	...
	if($_POST['action']=="blacklistIPJS")
	{
		if(current_user_can( 'manage_options' ))
		{
			...
			$IP=mysql_real_escape_string($_POST['blacklist']);
...
function IPBLC_login_failed(){
	...
	$postedData="";
	//print_r($_SERVER);
	$visitorIP=mysql_real_escape_string($_SERVER['REMOTE_ADDR']);
	$visitor_time=mysql_real_escape_string($_SERVER['REQUEST_TIME']);
	$visitor_user_agent=mysql_real_escape_string($_SERVER['HTTP_USER_AGENT']);
